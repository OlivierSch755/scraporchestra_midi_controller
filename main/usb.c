...
static void midi_task_read_example(void *arg)
{
    ...
    for (;;) {
        ...
        while (tud_midi_available()) {
            ...
            if (read) {
                ESP_LOGD(TAG, "Read - Time (ms since boot): %lld, Data: %02hhX %02hhX %02hhX %02hhX",
...
esp_err_t init_usbmidi(){
    ...
    ESP_LOGD(TAG, "USB initialization");
    ...
    ESP_LOGD(TAG, "USB initialization DONE");

    // Read received MIDI packets
    ESP_LOGD(TAG, "MIDI read task init");
